...
The user-maintained Artist Source Registry is the editorial source for these
thirteen artists. This migration preserves all monitoring settings already in
config/artists.json and only overlays verified public links. It also corrects
the current Beyond The Walls 3 listing using the official Ticketmaster lineup
and Brenno's official show calendar.
"""
...
BEYOND_THE_WALLS = {
    "id": "supplemental:beyond-the-walls-3-brenno-2026",
    "title": "Beyond The Walls 3",
    "startDate": "2026-11-07",
    "startTime": "19:30",
    "timezone": "America/New_York",
    "venue": "The Cambridge Room at House of Blues Cleveland",
    "address": "308 Euclid Ave",
    "city": "Cleveland",
    "state": "OH",
    "country": "US",
    "artists": ["KB", "Brenno", "Porsha Love"],
    "headliner": "KB",
    "eventType": "concert",
    "status": "onsale",
    "ticketUrl": "https://www.ticketmaster.com/beyond-the-walls-3-cleveland-ohio-11-07-2026/event/05006488EE58E30A",
    "officialUrl": "https://www.ticketmaster.com/beyond-the-walls-3-cleveland-ohio-11-07-2026/event/05006488EE58E30A",
    "image": "",
    "price": "",
    "sourceName": "Ticketmaster / Brenno official upcoming shows",
    ...
    "sources": [
        {
            "name": "Ticketmaster",
            "url": "https://www.ticketmaster.com/beyond-the-walls-3-cleveland-ohio-11-07-2026/event/05006488EE58E30A",
            "type": "manual_verified",
            "authority": "venue_ticket",
            "priority": 100,
        },
        {
            "name": "Brenno official upcoming shows",
            "url": "https://www.brennomusic.live/",
            "type": "manual_verified",
...
def is_beyond_walls(event: dict[str, Any]) -> bool:
    return (
        str(event.get("startDate") or "") == "2026-11-07"
        and norm(event.get("city")) == "cleveland"
        and "beyond the walls" in norm(event.get("title"))
    )
...
def main() -> int:
    ...
    for name, links in SOCIALS.items():
        ...
        if not artist:
            raise SystemExit(f"Artist missing from production registry: {name}")
        artist.update(links)

    corrected_events: list[dict[str, Any]] = []
    found_ticketmaster = False
    for event in events:
        if not is_beyond_walls(event):
            corrected_events.append(event)
            continue

        # Apple/Bandsintown currently associates Mike Teezy with this date, but
        # Ticketmaster's official lineup does not. Keep the stronger venue/ticket listing.
        ...
        if norm(event.get("sourceName")) == "ticketmaster":
            ...
            if "Brenno" not in names:
                try:
                    names.insert(names.index("KB") + 1, "Brenno")
                except ValueError:
                    names.append("Brenno")
            event["artists"] = names
        corrected_events.append(event)

    if not found_ticketmaster:
        raise SystemExit("Expected Ticketmaster Beyond The Walls 3 record was not found")
    ...
    write(EVENTS_FILE, corrected_events)
    ...
    for name in SOCIALS:
        ...
        for required in ("instagramProfile", "spotifyProfile", "youtubeProfile"):
            if not artist.get(required):
                raise SystemExit(f"{name} missing {required} after registry sync")

    live_beyond = [item for item in corrected_events if is_beyond_walls(item)]
    if len(live_beyond) != 1 or "Brenno" not in live_beyond[0].get("artists", []):
        raise SystemExit("Beyond The Walls 3 did not normalize to one Brenno-inclusive primary record")
