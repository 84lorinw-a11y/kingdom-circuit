...
class ArtistWebsiteIngestionTests(unittest.TestCase):
    ...
    def test_verified_websites_are_synced_to_roster(self):
        websites = json.loads((ROOT / "config" / "verified-artist-websites.json").read_text(encoding="utf-8"))
        artists = json.loads((ROOT / "config" / "artists.json").read_text(encoding="utf-8"))
        by_name = {artist.get("name", "").casefold(): artist for artist in artists}
        for name, website in websites.items():
            artist = by_name[name.casefold()]
            self.assertEqual(website, artist.get("website"), name)
            self.assertTrue(artist.get("websiteRegistryVerified"), name)
