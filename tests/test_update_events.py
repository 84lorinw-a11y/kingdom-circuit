...
class KingdomCircuitV7Tests(unittest.TestCase):
    ...
    def test_master_roster_has_365_unique_artists(self):
        artists = json.loads((ROOT / "config" / "artists.json").read_text())
        names = [item["name"] for item in artists]
        self.assertEqual(len(names), 365)
        self.assertEqual(len({name.casefold() for name in names}), 365)
        ...
        self.assertEqual(sum(1 for item in artists if item.get("monitoringPriority") == 3), 129)
