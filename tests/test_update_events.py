...
class KingdomCircuitV7Tests(unittest.TestCase):
    ...
    def test_master_roster_has_318_unique_artists(self):
        artists = json.loads((ROOT / "config" / "artists.json").read_text())
        names = [item["name"] for item in artists]
        self.assertEqual(len(names), 318)
        self.assertEqual(len({name.casefold() for name in names}), 318)
        self.assertEqual(sum(1 for item in artists if item.get("monitoringPriority") == 1), 102)
        self.assertEqual(sum(1 for item in artists if item.get("monitoringPriority") == 2), 111)
        self.assertEqual(sum(1 for item in artists if item.get("monitoringPriority") == 3), 105)
